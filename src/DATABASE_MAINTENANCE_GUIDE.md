...
1.  **El Orquestador (`database/SqliteManager.js`):** El punto de control central que gestiona la conexión y la configuración.
2.  **Los Planos (`database/Model/`):** El directorio que define la estructura de cada tabla. Es la "fuente de la verdad".
3.  **La Caja Fuerte (`Data/MarIADono3DB.sqlite`):** El archivo físico que contiene todos los datos.

---

## 1. El Orquestador: `database/SqliteManager.js`
...
*   **Responsabilidades Clave:**
    ...
    2.  **Cargar los Modelos:** Descubre y registra automáticamente todos los archivos de definición de tablas (modelos) que se encuentran en la carpeta `database/Model/`.
...
## 2. Los Planos: El Directorio `database/Model/`
...
| :--- | :--- | :--- |
| `database/SqliteManager.js` | **Conexión y Configuración** | Para depurar problemas de conexión o activar logs. |
| `database/Model/` | **Definición de Tablas** | **Siempre** que necesitemos cambiar la estructura de la BD. |
